...
struct CheckoutView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(item: $activePaymentItem) { item in
            PaymentWebView(url: item.url, successUrlKeyword: "order-complete") {
    ...
    private func startPaymentProcess() {
            ...
            Task {
                if let redirectURL = await PaymentService.shared.readyKakaoPay(request: request),
...
struct PricingBreakdown: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if hasCoupon {
                PricingRow(title: "쿠폰 할인 금액", amount: "-\(discount.formatted())원", color: .red)
            }
            Divider()
            HStack {
                Text("결제 예정 금액").font(.headline)
                Spacer()
                Text("\(finalPrice.formatted())원")
                    .font(.title3)
                    .fontWeight(.bold)
